...
export function BubbleTail({ side, warm = false }: { side: 'left' | 'right'; warm?: boolean }) {
  ...
  return (
    ...
    <div
      className={`absolute bottom-0 h-5 overflow-visible ${isLeft ? '-left-5' : '-right-5'}`}
      style={{ width: '25px', backdropFilter: 'blur(16px) saturate(150%)' }}
    >
      <svg
        aria-hidden="true"
        viewBox="0 0 25 20"
      ...
      >
        {/* x=20..25 — те же 5px, что и CSS-радиус угла пузыря (см. style
            ChatBubble ниже): полоса, где приближение цвета вообще видно
            (риск из #10j), сжата с 12px до 5px. Точная дуга (SVG `A`)
            здесь НЕ подошла — она жёстко фиксирует касательную на конце
            (вертикаль, потому что дуга касается левого края пузыря), а
            следующему сегменту хвостика там уже нужно направление НАРУЖУ —
            получался залом ровно в точке стыка (виден на 800%). Кубическая
            кривая вместо дуги даёт свободу выбрать контрольные точки так,
            чтобы касательная плавно поворачивала, а не ломалась. x<20 —
            сам хвостик, целиком снаружи пузыря. Обводка — тот же путь без
            прямой замыкающей кромки (25,17)→(25,20): та кромка лежит на
            стыке с плоским низом пузыря и не должна нести свою линию
            поверх него. */}
        <path
          d="M25 20 C25 15 22 12 17 14 C12 15 6 17 2 17 C6 19 15 18 25 17 Z"
          fill="var(--tail-fill)"
        />
        {warm && (
          <>
            <defs>
              <radialGradient id={gradientId} cx="0.9" cy="0.85" r="0.7">
                <stop offset="0%" stopColor="oklch(0.72 0.17 55 / 0.35)" />
        ...
        <path
          d="M25 20 C25 15 22 12 17 14 C12 15 6 17 2 17 C6 19 15 18 25 17"
          fill="none"
    ...
    >
      ...
      <motion.div
        style={{
          ...
          // Угол, из которого растёт хвостик, — МАЛЫЙ (5px, было 28.8px
          // --radius-2xl, затем 0px). 0px давал острую точку — хвостик
          // визуально «начинался с угла», а не вырастал из скругления.
          // 5px — тот же радиус, что повторяет SVG-хвостик у своего
          // корня (#10l) — CSS-угол и SVG-дуга описывают ОДНУ и ту же
          // окружность, поэтому переход между ними не читается глазом:
          // это не два разных скругления, встретившихся на границе, а
          // одна дуга, часть которой рисует браузer (точный цвет
          // градиента), а часть — SVG (приближение). Только у ПОСЛЕДНЕЙ
          // реплики группы (хвостик и аватар растут снизу, как в
          // WhatsApp/Telegram/iMessage): более ранние реплики пачки
