...
/** Хвостик пузыря: оттяжка к говорящему, переходящая в тело кривой Безье.
    ...
    видимый залом. У реального WhatsApp (перепроверено по своей же
    трассировке пикселей) касательная в этой точке — вертикальная, кривая
    сперва «утекает» вниз (лёгкая капля ниже линии низа пузыря — ровно то,
    что описал пользователь), и только потом заворачивает влево. Первый
    control-point теперь (20,24) — строго ПОД стартовой точкой, тот же
    вектор, что и прямое ребро над ней: кривая продолжает движение ребра,
    а не срывается с него под углом. */
  ...
  return (
    ...
    >
      ...
      >
        ...
        <path
          d="M20 20 C20 24 10 22 3 17 C3 13 8 9 14 8 C17 7 19 8 20 10 Z"
          fill="var(--tail-fill)"
        />
        {warm && (
          <>
            <defs>
              <radialGradient id={gradientId} cx="0.85" cy="0.9" r="0.75">
        ...
        <path
          d="M20 20 C20 24 10 22 3 17 C3 13 8 9 14 8 C17 7 19 8 20 10"
          fill="none"
