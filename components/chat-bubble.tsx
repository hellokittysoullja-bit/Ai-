...
/** Хвостик пузыря: оттяжка к говорящему, переходящая в тело кривой Безье.
    ...
    #10d — ФОРМА ЗАМЕНЕНА НА ОДИН КРУГ, ПЕРЕНЕСЁН ЗА .glass-shine: пользователь
    обвёл красным реальный хвостик WhatsApp — у него характерный «зубчик»-
    вырез снизу между двумя дугами, не гладкая капля. При попытке повторить
    это кругом поверх скруглённого угла всплыл настоящий баг: .glass-shine
    (см. className пузыря выше) несёт overflow:hidden ради диагонального
    блика — любой потомок с отрицательным отступом обрезался по границе
    пузыря. Пять прошлых версий формы поэтому «выглядели» рабочими на
    рендере, но НИКОГДА не высовывались за край — то, что казалось
    хвостиком, было видимым остатком фигуры ВНУТРИ пузыря. Хвостик теперь
    отдельный элемент РЯДОМ с .glass-shine (не внутри), с своим style={{x}}
    для синхронного движения при swipe-to-reply, и --tail-fill/--tail-stroke
    подняты на общего родителя (кастомные CSS-свойства не наследуются между
    соседями). Круг честно пересекает угол пузыря, но точный «зубчик» с
    двумя разными радиусами дуг подбором параметров вручную (без векторного
    редактора) не поймался — текущая форма читается как аккуратный
    слитный хвост, не идеальная копия скриншота. */
export function BubbleTail({ side }: { side: 'left' | 'right' }) {
  ...
  return (
    <svg
      aria-hidden="true"
      viewBox="0 0 20 20"
      className={`absolute -bottom-[2px] h-5 w-5 ${isLeft ? '-left-[10px]' : '-right-[10px] scale-x-[-1]'}`}
    ...
    >
      ...
      <motion.div
        style={{
          ...
          // Угол, из которого растёт хвостик, сжат до 3px (было 28.8px,
          ...
          ...(isLastOfGroup
            ? { [isUser ? 'borderBottomRightRadius' : 'borderBottomLeftRadius']: '3px' }
            : {}),
        }}
