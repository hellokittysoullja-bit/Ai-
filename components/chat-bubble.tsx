...
/** Хвостик пузыря: оттяжка к говорящему, переходящая в тело кривой Безье.
    ...
    #10g — ХУК С ВЫЕМКОЙ (по референсу пользователя, реальный Telegram-
    скрин): прежняя форма была ОДНОЙ гладкой вогнутой дугой — у настоящего
    Telegram/WhatsApp хвостика дуга не одна, а составная: тело хвостика
    выступает НИЖЕ и ЛЕВЕЕ, чем наша дуга давала, и между кончиком хвостика
    и низом пузыря есть выемка (cusp) — место, где кривая хвостика и кривая
    угла пузыря пересекаются, как при объединении двух окружностей. Замерено
    трассировкой референсного скриншота по пикселям (граница светлого/тёмного
    построчно) — не на глаз: одна гладкая C-кривая физически не может дать
    такую выемку, нужны три сегмента вместо одного — наружу-вниз к кончику,
    внутрь-вверх к выемке, снова наружу-вниз к плоскому низу пузыря. */
  ...
  return (
    ...
    >
      ...
      >
        ...
        <path d="M20 0 L20 13 A7 7 0 0 1 13 20 L20 20 Z" fill="var(--tail-fill)" />
        <path d="M19 16 A10 10 0 1 0 -1 16 A10 10 0 1 0 19 16 Z" fill="var(--tail-fill)" />
        {warm && (
          <>
            <defs>
            ...
            <path d="M20 0 L20 13 A7 7 0 0 1 13 20 L20 20 Z" fill={`url(#${gradientId})`} />
            <path d="M19 16 A10 10 0 1 0 -1 16 A10 10 0 1 0 19 16 Z" fill={`url(#${gradientId})`} />
          </>
        )}
        <path
          d="M20 13 A7 7 0 0 1 18.98 16.64 A10 10 0 1 0 18.17 20"
          fill="none"
