...
export function BubbleTail({ side, warm = false }: { side: 'left' | 'right'; warm?: boolean }) {
  ...
  return (
    ...
    <>
      {/* Клин (щель CSS-угла, x:17..33) — ЦЕЛИКОМ внутри пузыря, поверх уже
          непрозрачного фона. НЕ несёт backdrop-filter: тот же div раньше
          покрывал всю высоту угла (0..33) и размывал сам ТЕКСТ реплики
          под собой (blur(16px) — не «под хвостиком», а буквально поверх
          букв, замечено на рендере: серый мутный прямоугольник поверх
          «Это наш чат…»). Раз клин не граничит со страницей — ему нечего
          стекленить, он просто закрашивает щель тем же приближением
          цвета. Обводки нет — обе кромки клина совпадают с уже
          нарисованным краем пузыря (CSS-дуга радиуса 16 и его нижний
          край), собственной линии поверх них клин не несёт. */}
      <div
        ...
        style={{
          ...
          transformOrigin: isLeft ? '16px center' : undefined,
        }}
      >
        <svg aria-hidden="true" viewBox="17 17 16 16" className="size-full" style={{ overflow: 'visible' }}>
          {/* Тёплый градиент — тот же id/координаты (userSpaceOnUse,
              ...
              по цвету (проверено рендером — было заметно). blur здесь
              намеренно НЕТ (см. соседний блок) — на стыке räzная
              обработка (blur vs без) читалась тем же швом. */}
          {warm && (
            <defs>
          ...
          <path d="M33 17 L33 33 L17 33 A16 16 0 0 0 33 17 Z" fill="var(--tail-fill)" />
          {warm && (
            <path d="M33 17 L33 33 L17 33 A16 16 0 0 0 33 17 Z" fill={`url(#${wedgeGradientId})`} />
          )}
        </svg>
      </div>
