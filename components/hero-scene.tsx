  ...
  return (
    ...
    >
      ...
      {/* луна с кратерами и дышащим гало.
          Позиция (500, 234): сдвинута с 546 после того, как замер
          getScreenCTM на реальном 390-wide мобильном вьюпорте показал —
          при исправленном (не переполненном) хиро diск луны помещался
          с запасом, но мягкое гало (r=110) резалось правым краем кадра
          почти наполовину (cx=546 давал правый край гало на ~435px при
          вьюпорте 390px). cx=500 держит гало в кадре почти целиком, диск
          остаётся там же по высоте — выше маскота, не над его лапами. */}
      <circle cx="500" cy="234" r="110" fill="url(#hs-moon-glow)">
        <animate
      ...
      <g fill={INK} opacity="0.12">
        <circle cx="491" cy="225" r="6.2" />
        <circle cx="510" cy="242" r="4.2" />
        <circle cx="497" cy="248" r="2.7" />
      </g>
