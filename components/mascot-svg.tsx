...
export function MascotSvg({
  ...
  /* Э1 · Взгляд за скроллом: на таче pointermove мёртв — существо следило
     ...
     Гейт на pointer:fine — обязателен, а не косметика: без него на
     десктопе одновременный скролл (трекпад) и движение мыши писали в ту
     же пружину py разом, и settle-таймер скролла (500мс) дёргал взгляд
     в центр поверх того, что курсор уже вёл в другую сторону — глаза
     заметно «косили» на долю секунды. На таче конфликта нет: pointermove
     там и так не стреляет, эффект курсора выше уже неактивен. */
  useEffect(() => {
